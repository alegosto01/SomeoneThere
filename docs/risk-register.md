...
| R015 | Verifier safety SOP missing for field visits | Safety | Critical | Low | Gate all in-person services behind a written SOP, insurance quote, check-ins, daylight rules, buddy rules for interiors, and abort rights. | Open | Alessandro |
...
| R018 | Report delivery is slower than the user's deposit deadline | Product / Operational | High | High | Use per-package SLAs: Basic listing review target under 4 hours, field checks next day only after dry runs prove the timings. | Open | Alessandro |
